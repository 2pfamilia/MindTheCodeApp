...
namespace MindTheCodeApp.Utils
{
    public class PopulateDb : BackgroundService
    {
        ...
        private async Task PopulateBook(CancellationToken stoppingToken)
        {
            ...
            {
                new() { Title = "The Name of the Wind", Description = "An epic fantasy novel about a legendary wizard who recounts his life story, from humble beginnings to epic battles against dark forces.", Category = _dbcontext.BookCategoryEntity.Single(b => b.Title == "Fantasy"), Count = 18, Author = _dbcontext.BookAuthorEntity.Single(ba => ba.Name == "Max Cooper"), Price = 14.99 },
                new() { Title = "The Da Vinci Code", Description = "A thrilling mystery novel that follows a symbologist and a cryptologist as they race to uncover a secret that could shake the foundations of Christianity.", Category = _dbcontext.BookCategoryEntity.Single(b => b.Title == "Mystery"), Count = 10, Author = _dbcontext.BookAuthorEntity.Single(ba => ba.Name == "Daniel Hill"), Price = 14.99 },
                new() { Title = "A Song of Ice and Fire", Description = "A gripping fantasy series that takes place in a vast, medieval-inspired world filled with political intrigue, war, and magic.", Category = _dbcontext.BookCategoryEntity.Single(b => b.Title == "Fantasy"), Count = 8, Author = _dbcontext.BookAuthorEntity.Single(ba => ba.Name == "Grace Kim"), Price = 19.99 },
                new() { Title = "Gone Girl", Description = "A psychological thriller that follows a husband and wife as their marriage unravels and a mystery unfolds.", Category = _dbcontext.BookCategoryEntity.Single(b => b.Title == "Mystery"), Count = 5, Author = _dbcontext.BookAuthorEntity.Single(ba => ba.Name == "Lily Chen"), Price = 12.99 },
                new() { Title = "The Time Traveler's Wife", Description = "A moving love story about a man with a rare genetic disorder that causes him to time travel unpredictably, and the woman who loves him through it all.", Category = _dbcontext.BookCategoryEntity.Single(b => b.Title == "Romance"), Count = 15, Author = _dbcontext.BookAuthorEntity.Single(ba => ba.Name == "Sarah Patel"), Price = 10.99 },
                new() { Title = "Dune", Description = "A classic science fiction novel set in a distant future where noble families fight for control of a desert planet called Arrakis, which holds the key to the galaxy's most valuable resource.", Category = _dbcontext.BookCategoryEntity.Single(b => b.Title == "Sci-Fi"), Count = 12, Author = _dbcontext.BookAuthorEntity.Single(ba => ba.Name == "Jason Lee"), Price = 11.99 },
                new() { Title = "The Girl with the Dragon Tattoo", Description = "A dark and suspenseful crime novel that follows an investigative journalist and a young computer hacker as they work together to solve a decades-old mystery.", Category = _dbcontext.BookCategoryEntity.Single(b => b.Title == "Crime"), Count = 7, Author = _dbcontext.BookAuthorEntity.Single(ba => ba.Name == "Jack Davis"), Price = 13.99 },
        ...
        private async Task PopulateOrder(CancellationToken stoppingToken)
        {
            ...
            {
                new() { User = _dbcontext.UserEntity.Single(u => u.UserId == 2), AddressInformation = _dbcontext.UserEntity.Include(a => a.AddressInformation).Single(u => u.UserId == 2).AddressInformation, Cost = 66.95},
                new() { User = _dbcontext.UserEntity.Single(u => u.UserId == 4), AddressInformation = _dbcontext.UserEntity.Include(a => a.AddressInformation).Single(u => u.UserId == 4).AddressInformation, Cost = 10.99},
                new() { User = _dbcontext.UserEntity.Single(u => u.UserId == 4), AddressInformation = _dbcontext.UserEntity.Include(a => a.AddressInformation).Single(u => u.UserId == 4).AddressInformation, Cost = 24.98, Active = false, Fulfilled = true},
            };
